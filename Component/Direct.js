...
export default function Direct() {
  ...
  const [disabled1, setDisabled1] = useState(0);
  const [disabled2, setDisabled2] = useState(0);
  ...
  //     return searchDate.includes(value);
  //   });
  //   
  ...
  async function directWalletSearch(e) {
    ...
    const filteredData = userData?.directIncome?.filter((item) => {
      const searchWallet = item?.address;
  ...
  const startDateFn = async () => {
    ...
    const filteredData = userData?.directIncome?.filter((item) => {
      const dateData = new Date(item?.incomeDate)?.toLocaleDateString();
      
      return (
        dateData >= new Date(start)?.toLocaleDateString() &&
        dateData <= new Date(end)?.toLocaleDateString()
      );
    });
      ...
      <section class="dashboard">
                ...
                >
                  <div class="accordion-item">
                    <h2 class="accordion-header reward-header">
                      <button
                        class="accordion-button  dash_drop"
                        type="button"
                    ...
                    >
                      <div class="accordion-body reward-body" id>
                        <ul>
                          <li className="active">
        ...
        <div class="dashboard-right">
          ...
          <div class="container-fluid">
            ...
            <div class="direct-row1">
              <div class="directrow1-left">
                <img src="/Direct Reward@2x.png" alt="" />
              </div>
              <div class="directrow11-right">
                <p>Direct Reward</p>
                <span>
                  Amount :<strong>{localStorage.getItem("direct")} </strong>
                </span>
              </div>
            </div>
            <div class="direct-row2">
              <div class="directrow2-head">
                <div class="drow2head-left">
                  <h4>Transaction History</h4>
                </div>
                <div class="drow2head-right">
                      ...
                      ? userData?.directIncome?.map((item, id) => {
                          return (
                            <tr>
                              <td>{id + 1}</td>
                              <td>
                              { format(new Date(item?.incomeDate), "dd" + " " + "MMMM" + " " + "yyyy") + " "  + "|" + " " + new Date(item.incomeDate).toLocaleString()?.slice(11,-3)}
                              </td>
                              <td>
                                {item?.firstName} {""} {item?.lastName}
                              </td>
                              <td class="incentive-copy" id="incentive-copy">
                                {item?.address}{" "}
                                <CopyToClipboard
                                  text={item?.address}
                                  // onCopy={() =>
