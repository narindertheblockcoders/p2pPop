...
export default function TwoFactor() {
  ...
  async function sendFaStatus(data) {
    try {
      const token = localStorage.getItem("token");
      let res = await axios.post("/api/sendStatus", { token: token, data });
      const response = res.data;
  ...
  async function disablesubmitHandler(fullCode) {
    const data = {
      otp: fullCode,
    };
    sendFaStatus(data);
  ...
  useEffect(() => {
    ...
    const asterikMobile = `${"******"}${suffix}`;
    setPhone(asterikMobile);
  }, []);
  ...
  function handleChangeFn8(e) {
    ...
    if (fullCode.length === 6) {
      setLengthError(false);
      verifySubmitHandler(fullCode);
      // disablesubmitHandler(fullCode)
    } else {
      setLengthError(true);
    }
  }

  function handleChangeFn9(e) {
    setCode3(e.target.value);
    num4.current.focus();
    const fullCode = `${code1}${code2}${e.target.value}${code4}${code5}${code6}`;
    if (e.target.value != NaN) {
    ...
    <>
            ...
            <div class="two-factors">
              ...
              <div class="factorlower-right">
                <label class="switch">
                  <input
                    type="checkbox"
                    checked={faStatus == 1 ? true : false}
                    onChange={(e) => handleModalFn(e)}
                  />
        ...
        <section class="two-factors1">
          <div class="container">
            <div class="twofactor-content">
              ...
              <p>Download the free google authenticator app</p>
              <p>Set Up a new account in this app</p>
              <p>Scan this barcode or tye the key manually</p>
              <div class="qrscan">
                <img src={qrImage?.qrData}></img>

                <div class="txtwtcopy">
                  <span>
                    {qrImage?.base}

                    <CopyToClipboard text={qrImage?.base}>
                      <Tooltip
                        content={"Copied"}
                        trigger="click"
                        color="primary"
      ...
      >
        ...
        <button type="button" class="btn-close" aria-label="Close">
          <img onClick={()=>setShow2(false)} src="/close.png"/>
        </button>
        </ModalHeader>
          <div class="container">
            <div class="twofactor-content">
              ...
              <h6 style={{
                marginTop:"30px"
              }}>Enter the 6-digit code that you see in the app</h6>
              <p className="mt-0 mb-3" style={{ color: "#1F74F2" }}>Enter Code</p>
              ...
              {/* {numericError ? (
                ...
                >
                  Code should be in numeric only!
                </p>
              ) : (
                ""
              )} */}
            ...
            <div class="twofactor-content">
              ...
              <Link href="/dashboard">
                <div class="success-done">
                  <button class="connect-wallet">Done</button>
                </div>
              </Link>
            </div>
          </div>
      ...
      >
        ...
        <ModalHeader >
        <button type="button" class="btn-close" aria-label="Close">
          <img onClick={()=>setShow4(false)} src="/close.png"/>
        </button>
        </ModalHeader>
          <div class="container"> 
            <div class="twofactor-content">
              <h3 className="mt-0">Verify</h3>
              <h6 className="mt-0">
                Enter the 6-digit code that you see in the app to disable Two
                Factor Authentication
              </h6>
              <p className="mb-3">Enter Code</p>
              <div class="twofactor-code">
                <div class="mb-3">
                  <input
                    type="text"
